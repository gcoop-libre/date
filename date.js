     ...
     if (type == 'date_default') {
       ...
       $.each(items, function(delta, item) {
         ...
         if (value2_present) {
           ...
           var from_day = date_translate(date(format_day, d.getTime()));
           var to_day = date_translate(date(format_day, d2.getTime()));
           ...
           if (from_day == to_day) {
             element[delta] = {
               markup: '<div class="value">' + from_day + ' - ' + from_hour + ' ' + t('to') + ' ' + to_time + '</div>'
             };
           } else {
             var label = value2_present ? t('From') + ': ' : '';
             element[delta] = {
               markup: '<div class="value">' + label + date_translate(date(format_full, d.getTime())) + '</div>'
             };
             element[delta].markup += '<div class="value2">' + t('To') + ': ' + date_translate(date(format_full, d2.getTime())) + '</div>';
           }
         } else {
           element[delta] = {
             markup: '<div class="value">' + label + date_translate(date(format_full, d.getTime())) + '</div>'
